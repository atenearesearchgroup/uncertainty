...
//		System.out.println("b5="+b5+" p="+b5.projection()+" mu="+b5.uncertaintyMaximized()+" uBoolean="+b5.toUBoolean());
	...
	void otherTests2() {
		Collection<SBoolean> opinions = new ArrayList<SBoolean>(); 
		SBoolean adam = new SBoolean(0.0,0.95,0.05,0.5);
		SBoolean beth = new SBoolean(0.0,0.33,0.67,0.5);
		SBoolean charles = new SBoolean(0.67,0.0,0.33,0.5);
		SBoolean diana = new SBoolean(0.95,0.0,0.05,0.5);

		for (int i=0;i<18;i++) opinions.add(adam); 
		for (int i=0;i<114;i++) opinions.add(beth); 
		for (int i=0;i<33;i++) opinions.add(charles); 
		for (int i=0;i<34;i++) opinions.add(diana); 
		
	    SBoolean acf = SBoolean.averageBeliefFusion(opinions);test0(acf);
	    SBoolean wcf = SBoolean.weightedBeliefFusion(opinions);test0(wcf);
	...
	void otherTests3() {
		// System.out.println("other tests");
		SBoolean x,y;
		SBoolean V=new SBoolean(0,0,1,0.5);
		for(double u=0;u<=10;u++) {
			for(double b=0;b<=10;b++) {
				for(double a=0;a<=10;a++)
					if (u/10+b/10 <= 1) {
						x=new SBoolean(b/10,1-u/10-b/10,u/10,a/10);
						//y=x.weightedFusion(V);
						y=x.averageFusion(V);
						//assertEquals(x,x.averageFusion(V));
						// assertEquals(x.b,y.b);
						// assertEquals(x.d,y.d);
						// assertEquals(x.u,y.u);
					}
			}
		}	
	}
	
	void wuTest() { //weightedUnion() 
		List<SBoolean> l1 = new ArrayList<SBoolean>();
