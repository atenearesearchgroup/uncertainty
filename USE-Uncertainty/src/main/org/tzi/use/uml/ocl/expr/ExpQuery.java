...
public abstract class ExpQuery extends Expression {
    ...
    protected final Value evalUSelect(EvalContext ctx) {
        ...
        double confident = evalAndAsertConfident(ctx);
    ...
    private double evalAndAsertConfident(EvalContext ctx) {
    ...
    protected final Value evalExistsOrForAll(EvalContext ctx, boolean doExists) {
        ...
        if (!doExists)
            res = evalForAll0(0, rangeVal, ctx);
        else
            res = evalExists0(0, rangeVal, ctx);
    ...
    private final Value evalExists0(int nesting, CollectionValue rangeVal, EvalContext ctx) {
        ...
        for (Value elemVal : rangeVal) {
            ...
            if (!fElemVarDecls.isEmpty() && nesting < fElemVarDecls.size() - 1) {
                ...
                // eventually
                res = evalExists0(nesting + 1, rangeVal, ctx);
            }
            else {
                // evaluate predicate expression
                Value queryVal = fQueryExp.eval(ctx);

                // Undefined query values are set by default false
                if (queryVal.isUndefined())
                    queryVal = BooleanValue.FALSE;

                try {
                    expOr = ExpStdOp.create("or", new Expression[]{
                            new ExpressionWithValue(res),
                            new ExpressionWithValue(queryVal)
                    });
                    res = expOr.eval(ctx);
                }
                catch (ExpInvalidException ex) {
                    // This might not happen, so I set it false to continue iteration
    ...
    private final Value evalForAll0(int nesting, CollectionValue rangeVal, EvalContext ctx) {
        ...
        for (Value elemVal : rangeVal) {
            ...
            if (!fElemVarDecls.isEmpty() && nesting < fElemVarDecls.size() - 1) {
                ...
                // assigning each value to each element variable
                // eventually
                res = evalForAll0(nesting + 1, rangeVal, ctx);
            }
            else {
                // evaluate predicate expression
                Value queryVal = fQueryExp.eval(ctx);
                ...
                try {
                    expAnd = ExpStdOp.create("and", new Expression[]{
                            new ExpressionWithValue(res),
                            new ExpressionWithValue(queryVal)
                    });
                    res = expAnd.eval(ctx);
                    ...
                    // This might not happen, so I set it true to continue iteration
    ...
    /**
     * @deprecated Since that needed UBoolean support.
     * @see {@link #evalForAll0(int, CollectionValue, EvalContext)}
     * @see {@link #evalExists0(int, CollectionValue, EvalContext)}
     */
